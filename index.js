...
const total_contributions = GAMES_JSON.reduce((acc, curr_game) => acc + curr_game.backers, 0);
...
const total_raised = GAMES_JSON.reduce((acc, curr_game) => acc + curr_game.pledged, 0);
...
const displayStr = `A total of $${total_raised} has been raised for ${GAMES_JSON.length} games. 
Currently, ${unfundedNum} ${unfundedNum > 1 ? "games" : "game"} ${unfundedNum > 1 ? "remain" : "remains"} unfunded. 
...
document.getElementById('search-input').addEventListener('keyup', function() {
    ...
    const items = GAMES_JSON;
   
    const new_items = items.filter(item => item.name.toLowerCase().includes(query))
    deleteChildElements(gamesContainer);
